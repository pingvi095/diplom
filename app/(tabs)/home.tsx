...
export default function Home() {
  ...
  const [distanceSort, setDistanceSort] = useState<DistanceFilter>('default')
  ...
  const loadFavorites = async () => {
    ...
    setFavorites((data || []).map((i) => String(i.event_id)))
  }
  ...
  const eventsWithDistance = useMemo(() => {
    return events.map((e) => {
      if (currentLocation && e.latitude != null && e.longitude != null) {
        return {
          ...e,
  ...
  const filtered = useMemo(() => {
    ...
    if (age !== 'all') {
      if (age === 'none') {
        list = list.filter((e) => !e.age_limit)
      } else {
        list = list.filter((e) => Number(e.age_limit) === Number(age))
      }
    }
        ...
        <View style={styles.modalContainer}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.grid}>
              <FilterChip label="Все" active={age === 'all'} onPress={() => setAge('all')} />
              <FilterChip
                label="Без ограничения"
                active={age === 'none'}
                onPress={() => setAge('none')}
              />
              <FilterChip label="12+" active={age === '12'} onPress={() => setAge('12')} />
              <FilterChip label="16+" active={age === '16'} onPress={() => setAge('16')} />
              <FilterChip label="18+" active={age === '18'} onPress={() => setAge('18')} />
              <FilterChip label="21+" active={age === '21'} onPress={() => setAge('21')} />
            </View>
